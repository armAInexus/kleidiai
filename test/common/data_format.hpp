...
class DataFormat {
public:
    /// Quantization packing format.
    enum class QuantizationFormat : uint32_t {
        NONE,     ///< No quantization information is included.
        PER_ROW,  ///< Per-row quantization.
    };
    ...
    /// @param[in] quant_format Quantization packing format.
    ...
    DataFormat(
        DataType data_type, size_t block_height = 0, size_t block_width = 0,
        QuantizationFormat quant_format = QuantizationFormat::NONE, DataType scale_dt = DataType::UNKNOWN,
        DataType zero_point_dt = DataType::UNKNOWN, size_t subblock_height = 0, size_t subblock_width = 0) noexcept;
    ...
    /// Gets the quantization packing format.
    [[nodiscard]] QuantizationFormat quantization_format() const;
    ...
    /// Gets a value indicating whether this format has no blocking or packed quantization information.
    ...
    ///
    /// In case of per-row quantization, the row stride is the number of bytes from one row group
...
private:
    DataType _data_type;
    QuantizationFormat _quant_format;
    DataType _scale_dt;
