...
struct kai_rhs_pack_qsi4cxp_qsu4cxs1s0_params {
...
/// the int4 values in a N x K matrix (if is_rhs_nxk = true), or in a K x N matrix (if is_rhs_nxk = false).
/// In the RHS matrix (not packed), N is number of rows if is_rhs_nxk = true or the number of columns if is_rhs_nxk =
/// false. In the RHS matrix (not packed), K is the number of columns if is_rhs_nxk = true or the number of rows if
/// is_rhs_nxk = false. Two int4 values are stored in one byte. The lower order part of the byte (low) holds the first
/// nibble (K-index + 0). The higher order of the byte holds the second nibble (K-index + 1).
///
/// @param[in] n_idx      Row index in the RHS matrix (not packed). It must be a multiple of n_step.
/// @param[in] is_rhs_nxk True if the RHS matrix not packed is N x K. It should be false otherwise.
...
///
/// @param[in] k     In the RHS matrix (not packed), K is the number of columns if is_rhs_nxk = true or the number of
/// rows if is_rhs_nxk = false.
/// @param[in] nr    The number of columns written by the matmul micro-kernel.
...
///
/// @note  The int4 values are stored in a N x K matrix (if is_rhs_nxk = true) or in a K x N matrix (if is_rhs_nxk =
/// false).
///        In the RHS matrix (not packed), N is number of rows if is_rhs_nxk = true or the number of columns if
///        is_rhs_nxk = false. In the RHS matrix (not packed), K is the number of columns if is_rhs_nxk = true or the
///        number of rows if is_rhs_nxk = false. Two int4 values are stored in one byte. The lower order part of the
///        byte (low) holds the first nibble (K-index + 0). The higher order of the byte holds the second nibble
///        (K-index + 1).
///
/// @param[in]  num_groups  The number of groups. It must be 1.
/// @param[in]  n           The number of rows if is_rhs_nxk = true or the number of columns if is_rhs_nxk = false.
/// @param[in]  k           The common dimension between the LHS and RHS matrix (K). It must be an even value.
/// @param[in]  nr          The number of N rows (if is_rhs_nxk = true) or colums (if is_rhs_nxk = false) to interleave
/// on the same output output row.
