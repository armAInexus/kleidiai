...
#include "kai_rhs_pack_kxn_f16p2vlx2b_f16_f16_sme.h"
...
size_t kai_get_n_step_rhs_pack_kxn_f16p2vlx2b_f16_f16_sme(void) {
    return kai_nr * kai_get_sme_vector_length_u16();
}

size_t kai_get_rhs_offset_rhs_pack_kxn_f16p2vlx2b_f16_f16_sme(size_t n_idx) {
    KAI_ASSUME(n_idx % (kai_nr * kai_get_sme_vector_length_u16()) == 0);

    return n_idx * kai_num_bytes_input;
}

size_t kai_get_bias_offset_rhs_pack_kxn_f16p2vlx2b_f16_f16_sme(size_t n_idx) {
    return n_idx * kai_num_bytes_bias;
}

size_t kai_get_rhs_packed_stride_rhs_pack_kxn_f16p2vlx2b_f16_f16_sme(size_t k) {
    return kai_nr * kai_get_sme_vector_length_u16() / kai_kr *
        (kai_num_bytes_bias + kai_roundup(k, kai_kr) * kai_num_bytes_output);
}

size_t kai_get_rhs_packed_offset_rhs_pack_kxn_f16p2vlx2b_f16_f16_sme(size_t n_idx, size_t k) {
    KAI_ASSUME(n_idx % (kai_nr * kai_get_sme_vector_length_u16() / kai_kr) == 0);

    return n_idx * (kai_num_bytes_bias + kai_roundup(k, kai_kr) * kai_num_bytes_output);
}

size_t kai_get_rhs_packed_size_rhs_pack_kxn_f16p2vlx2b_f16_f16_sme(size_t n, size_t k) {
    return kai_get_rhs_packed_offset_rhs_pack_kxn_f16p2vlx2b_f16_f16_sme(
        kai_roundup(n, kai_nr * kai_get_sme_vector_length_u16() / kai_kr), k);
}

void kai_run_rhs_pack_kxn_f16p2vlx2b_f16_f16_sme(
    ...
    KAI_ASSUME(nr == kai_nr * kai_get_sme_vector_length_u16());
    ...
    __asm__ __volatile__(
        ...
        "bgt 3b\n"
        "4:"  // Main row loop: Column loop skip
        "cmp %x[height], #0x8\n"
        "addvl %x[out], %x[out], #8\n"
        "bge 2b\n"
        "cbz %x[height], 9f\n"
        ...
        "add x28, x28, %x[out_stride]\n"
        "bgt 7b\n"
        "8:"  // Tail row loop: Column loop skip
        "cmp %x[height], #0x1\n"
        "addvl %x[out], %x[out], #2\n"
        "bge 6b\n"
        "9:"  // Done
        ...
        : "cc", "memory", "p0", "p1", "p2", "p3", "p4", "p5", "p6", "p7", "p8", "p9", "p10", "p11", "p12", "p13", "p14",
          "p15", "x9", "x20", "x21", "x22", "x23", "x24", "x25", "x26", "x27", "x28", "z0", "z1", "z2", "z3", "z4",
