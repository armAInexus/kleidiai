...
size_t kai_get_n_step_rhs_quant_pack_kxn_bf16p12x4biasf16_f16_neon(void) {
    return kai_nr;
}

size_t kai_get_rhs_offset_rhs_quant_pack_kxn_bf16p12x4biasf16_f16_neon(size_t n_idx) {
    KAI_ASSUME(n_idx % kai_nr == 0);

    return n_idx * kai_num_bytes_input;
}

size_t kai_get_bias_offset_rhs_quant_pack_kxn_bf16p12x4biasf16_f16_neon(size_t n_idx) {
    return n_idx * kai_num_bytes_bias;
}

size_t kai_get_rhs_packed_stride_rhs_quant_pack_kxn_bf16p12x4biasf16_f16_neon(size_t k) {
    return kai_nr * (kai_num_bytes_bias + kai_roundup(k, kai_kr) * kai_num_bytes_output);
}

size_t kai_get_rhs_packed_offset_rhs_quant_pack_kxn_bf16p12x4biasf16_f16_neon(size_t n_idx, size_t k) {
    KAI_ASSUME(n_idx % kai_nr == 0);

    return n_idx * (kai_num_bytes_bias + kai_roundup(k, kai_kr) * kai_num_bytes_output);
}

size_t kai_get_rhs_packed_size_rhs_quant_pack_kxn_bf16p12x4biasf16_f16_neon(size_t n, size_t k) {
    return kai_get_rhs_packed_offset_rhs_quant_pack_kxn_bf16p12x4biasf16_f16_neon(kai_roundup(n, kai_nr), k);
}

void kai_run_rhs_quant_pack_kxn_bf16p12x4biasf16_f16_neon(
    ...
    const __fp16* pad_row = rhs;

    // Fill zeros if bias is nullptr
    size_t bias_step = nr * sizeof(__fp16);
    uint8_t zero_bias[MAX_NR * sizeof(__fp16)];

    if (bias == NULL) {
        memset(zero_bias, 0, MAX_NR * sizeof(__fp16));
        bias_step = 0;
    }

    const void* bias_ptr = bias == NULL ? (void*)zero_bias : (void*)bias;

    size_t out_stride = kai_get_rhs_packed_stride_rhs_quant_pack_kxn_bf16p12x4biasf16_f16_neon(height);
